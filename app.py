...
ma_coords = {
    ...
    "Boston": (42.360, -71.058), "Brockton": (42.083, -71.018), "Fitchburg": (42.583, -71.802), "Springfield": (42.101, -72.589),
    "Ludlow": (42.160, -72.474), "Methuen": (42.726, -71.190), "Lee": (42.304, -73.249), "Worcester": (42.262, -71.802)
}
...
if not data.empty:
    for _, row in data.iterrows():
        ...
        folium.CircleMarker([offset_lat, offset_lon], radius=6, color=color, fill=True, fill_opacity=0.8, tooltip=folium.Tooltip(tooltip_html)).add_to(m)
...
tab1, tab2, tab3 = st.tabs(["🤝 CX & Sales (SLA Engine)", "🏛️ Policy (DPU Tracker)", "📈 YoY Financial Trends"])
...
with tab1:
    ...
    utility_sla = utility_sla.rename(columns={'Cycle Time': 'Predicted Timeline (Invoice to PTO)'}).sort_values(by='Predicted Timeline (Invoice to PTO)', ascending=False)
