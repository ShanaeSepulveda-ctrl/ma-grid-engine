...
def load_and_clean_data():
    ...
    try:
        # UPDATED to your newest file with Managers
        ...
        df_comp['Sales_Team'] = df_comp['Sales_Team'].fillna("Unassigned").astype(str)
        df_comp['Sales_Rep'] = df_comp['Sales_Rep'].fillna("Unknown").astype(str)
        df_comp['Utility'] = df_comp['Utility'].fillna("Unknown").astype(str)
    except Exception as e:
        df_comp = pd.DataFrame() # Fallback if missing
        
    # 2. Load Transformer Upgrades (For Grid Risk & Costs)
    try:
...
st.sidebar.header("👤 Individualize View")
if not df_comp.empty:
    # Hierarchical filtering: Manager -> Team -> Rep
    all_managers = sorted(df_comp['Manager'].unique().tolist())
    manager_filter = st.sidebar.multiselect("Sales Manager", all_managers, default=all_managers)

    filtered_teams = df_comp[df_comp['Manager'].isin(manager_filter)]['Sales_Team'].unique().tolist()
    team_filter = st.sidebar.multiselect("Sales Team", sorted(filtered_teams), default=sorted(filtered_teams))

    filtered_reps = df_comp[(df_comp['Manager'].isin(manager_filter)) & (df_comp['Sales_Team'].isin(team_filter))]['Sales_Rep'].unique().tolist()
    rep_filter = st.sidebar.multiselect("Sales Rep", sorted(filtered_reps), default=sorted(filtered_reps))

    all_utilities = sorted(df_comp['Utility'].unique().tolist())
    utility_filter = st.sidebar.multiselect("Utility Company", all_utilities, default=all_utilities)

    # Apply Filters to Timeline Data
    filtered_comp = df_comp.copy()
    filtered_comp = filtered_comp[filtered_comp['Manager'].isin(manager_filter)]
    filtered_comp = filtered_comp[filtered_comp['Sales_Team'].isin(team_filter)]
    filtered_comp = filtered_comp[filtered_comp['Sales_Rep'].isin(rep_filter)]
    filtered_comp = filtered_comp[filtered_comp['Utility'].isin(utility_filter)]
else:
    filtered_comp = pd.DataFrame()

st.sidebar.info("Map and KPIs display active grid risks and historical speeds to help you set perfect expectations.")
...
if search_city:
    ...
    with st.expander("Universal Objection Handlers (Click to view)"):
        st.markdown("**Objection: 'I want to wait a few weeks before signing.'**\n> \"I hear you. The only catch is that the local permitting queue does not wait. Every day we delay submitting your paperwork is another day added to the back end of the city's timeline. Let's get the administrative work started now so you aren't stuck waiting later.\"")
        st.markdown("**Objection: 'Why does it take so many days just to get a permit?'**\n> \"It is a great question. We are essentially building a custom power plant on your roof. Your city's building department and your utility company have to review the structural and electrical engineering to ensure everything is perfectly safe and up to code. We handle all of that complex paperwork for you, but we do have to wait for them to process it at their own speed.\"")
else:
...
# --- TIMELINE & RISK MAP ---
st.subheader("Interactive Grid Intelligence Map")
st.caption("Dots represent active Transformer Upgrades. Hover to see grid risk and average historical timeline.")
...
if not df_tu.empty:
    # Precalculate city timelines from the completed dataset to show on the TU map
    ...
    for _, row in df_tu.iterrows():
        ...
        cost = row['TU_Cost']
        if cost > 10000: fill_color = "#FF3D00"   # Red
        elif cost > 5000: fill_color = "#FFC107"  # Yellow
        else: fill_color = "#00E676"              # Green
